...
function listenOnPort(port) {
  ...
  app.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.log(`Port ${port} sudah dipakai, coba pakai port yang laen...`);
      listenOnPort(port + 1);
    } else {
      console.error(err);
    }
...
function start(file) {
  ...
  p.on("message", (data) => {
    console.log('\x1b[36m%s\x1b[0m', `🟢 dapet ${data}`);
    switch (data) {
      case "reset":
        p.kill();
        isRunning = false;
  ...
  p.on("exit", (code) => {
    ...
    console.error('\x1b[31m%s\x1b[0m', `File ${args[0]} udah dimodif. Script bakal restart...`);
      start("main.js");
    });
  });
  ...
  p.on("error", (err) => {
    ...
    p.kill();
    isRunning = false;
    console.error('\x1b[31m%s\x1b[0m', `Error occurred. Script bakal restart...`);
    start("main.js");
  });
  ...
  fs.readdir(pluginsFolder, (err, files) => {
    if (err) {
      console.error('\x1b[31m%s\x1b[0m', `Error pas baxa folder plugins: ${err}`);
      return;
    }
  ...
  console.log('\x1b[33m%s\x1b[0m', `📃 Script Ori by BOTCAHX and Recode By Lann and Recode Again By bluxsxx`);
